...
const SUMMARY_GROWTH_CONFIG = require('../config/summaryGrowthConfig');
...
let excelCache = { mtimeMs: 0, data: null };
...
router.post('/excel', protect, upload.single('file'), (req, res) => {
  try {
    bootstrapDefaultData();
    if (!req.file) {
    ...
    const { sheets, industries } = parseWorkbookBuffer(req.file.buffer);
    const uploadedAt = new Date().toISOString();
    const { nextStatus, tempDataset } = withStorageLock(() => {
      const status = clearTempDataset(readStatus());
      const { originalName, stagedFilename } = stageUploadedFile(req.file, uploadedAt);
      const nextTempDataset = createDatasetMeta({
        filename: 'temp_uploaded_data.json',
        originalName,
        uploadedAt,
        persistedAt: uploadedAt,
        size: req.file.size,
        rawSize: req.file.size,
        stagedFilename,
      });

      writeJson(TEMP_DATA_PATH, industries);
      const updatedStatus = {
        ...status,
        tempDataset: nextTempDataset,
      };
    ...
    return res.json({
      success: true,
      message: `Preview ready for "${tempDataset.originalName}"`,
      sheets,
      industries,
      tempDataset,
      defaultDataset: nextStatus.defaultDataset,
    });
    ...
    try {
      ...
      // Step 1: Back up the existing active file before saving the new one
      if (registry.active) {
        const currentActivePath = path.join(ACTIVE_DIR, registry.active.filename);
        if (fs.existsSync(currentActivePath)) {
          const backupFilename = `${safeTimestamp()}_${registry.active.originalName}`;
          const backupPath = path.join(BACKUP_DIR, backupFilename);
          // Copy first — if this throws, we abort before touching the active file
          fs.copyFileSync(currentActivePath, backupPath);
          backupCreated = {
            filename: backupFilename,
            originalName: registry.active.originalName,
            backedUpAt: now.toISOString(),
            uploadedAt: registry.active.uploadedAt,
            size: registry.active.size,
          };
          registry.backups.unshift(backupCreated);
          fs.unlinkSync(currentActivePath);
        }
      }

      // Step 2: Write new file as the active default
      const activeFilename = req.file.originalname;
      fs.writeFileSync(path.join(ACTIVE_DIR, activeFilename), req.file.buffer);
      ...
      console.error('[FileBackup] Error:', saveErr.message);
      return res.status(500).json({
        success: false,
        message: `Backup failed: ${saveErr.message}. Upload aborted to protect current data.`,
      });
    }
    // ───────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  GET /api/upload/backups  — list active file and all backups
// ─────────────────────────────────────────────────────────────────────────────
router.get('/data', protect, (req, res) => {
  try {
    const activeFilePath = path.join(ACTIVE_DIR, 'Economic Quarterly data Final.xlsx');
    if (fs.existsSync(activeFilePath)) {
      const stats = fs.statSync(activeFilePath);
      if (excelCache.mtimeMs !== stats.mtimeMs || !excelCache.data) {
        console.log('[Dashboard] Re-parsing active Excel file:', activeFilePath);
        const { industries } = parseWorkbookBuffer(fs.readFileSync(activeFilePath));
        excelCache.mtimeMs = stats.mtimeMs;
        excelCache.data = ensureMarketSectorInIndustries(industries);
      }
      return res.json({
        success: true,
        industries: excelCache.data,
        defaultDataset: {
    ...
    // Fallback to legacy behavior if the active Excel file is missing
    const status = getStatusSnapshot();
    return res.json({
      success: true,
      industries: ensureMarketSectorInIndustries(readJson(DEFAULT_DATA_PATH, [])),
      defaultDataset: status.defaultDataset,
    });
  } catch (err) {
    console.error('Error reading active Excel file:', err);
...
router.get('/status', protect, (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      defaultDataset: status.defaultDataset,
      tempDataset: status.tempDataset,
...
router.get('/backups', protect, (req, res) => {
  try {
    const status = getStatusSnapshot();
    return res.json({
      success: true,
      active: status.defaultDataset,
      tempDataset: status.tempDataset,
      backups: status.backups || [],
    });
  } catch (err) {
...
// ─────────────────────────────────────────────────────────────────────────────
//  POST /api/upload/restore  — restore a backup as the active default
// ─────────────────────────────────────────────────────────────────────────────
router.post('/discard-temp', protect, (req, res) => {
  try {
    const status = withStorageLock(() => {
      const nextStatus = clearTempDataset(getStatusSnapshot());
    ...
    return res.json({
      success: true,
      message: 'Temporary uploaded data discarded. Reverted to saved default dataset.',
      defaultDataset: status.defaultDataset,
    });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
...
router.post('/set-default', protect, (req, res) => {
  try {
    const result = withStorageLock(() => {
      bootstrapDefaultData();
      const status = readStatus();
      const tempIndustries = ensureMarketSectorInIndustries(readJson(TEMP_DATA_PATH, null));

      if (!tempIndustries || !status.tempDataset) {
        return {
          error: { code: 400, message: 'No temporary uploaded dataset is available to save.' },
        };
      }

      const nowIso = new Date().toISOString();
      const backupCreated = createBackupFromDefault(status, nowIso);
      const nextBackups = backupCreated ? [backupCreated, ...(status.backups || [])] : (status.backups || []);
      const fileReplacement = moveStageIntoActive(status.tempDataset, nowIso);

      writeJson(DEFAULT_DATA_PATH, tempIndustries);

      const nextStatus = clearTempDataset({
        ...status,
        defaultDataset: createDatasetMeta({
          filename: 'default_data.json',
          originalName: status.tempDataset.originalName,
          uploadedAt: status.tempDataset.uploadedAt,
          persistedAt: nowIso,
          setAsDefaultAt: nowIso,
          size: Buffer.byteLength(JSON.stringify(tempIndustries)),
        }),
        backups: nextBackups,
      });

      writeStatus(nextStatus);
      appendOperationLog('default_dataset_set', {
        originalName: nextStatus.defaultDataset.originalName,
        uploadedAt: nextStatus.defaultDataset.uploadedAt,
        setAsDefaultAt: nextStatus.defaultDataset.setAsDefaultAt,
        datasetBackup: backupCreated ? backupCreated.filename : null,
        fileBackup: fileReplacement.backupCreated ? fileReplacement.backupCreated.filename : null,
      });
      ...
      return {
        ...
        tempIndustries,
        backupCreated,
        fileBackupCreated: fileReplacement.backupCreated,
        activeFile: fileReplacement.activeFile,
      };
    });
    ...
    return res.json({
      success: true,
      message: `"${result.nextStatus.defaultDataset.originalName}" is now the default dataset.`,
      industries: result.tempIndustries,
      defaultDataset: result.nextStatus.defaultDataset,
      activeFile: result.activeFile,
      backupCreated: result.backupCreated,
      fileBackupCreated: result.fileBackupCreated,
      backups: result.nextStatus.backups,
    });
  } catch (err) {
    console.error('[SetDefault] Error:', err.message);
    return res.status(500).json({ success: false, message: `Failed to set default dataset: ${err.message}` });
  }
});

router.post('/restore-backup', protect, (req, res) => {
  const backupId = req.body.backupId || req.body.filename;
  ...
  try {
    ...
    const targetBackup = backupId
      ? backups.find((backup) => backup.id === backupId || backup.filename === backupId)
      : backups[0];

    if (!targetBackup) {
      return res.status(404).json({ success: false, message: 'Backup dataset not found.' });
    }

    const backupJsonPath = path.join(HISTORY_DIR, targetBackup.filename);
    const legacyBackupPath = path.join(BACKUP_DIR, targetBackup.filename || '');
    let restoredIndustries = ensureMarketSectorInIndustries(readJson(backupJsonPath, null));

    if (!restoredIndustries && fs.existsSync(legacyBackupPath)) {
      restoredIndustries = parseWorkbookBuffer(fs.readFileSync(legacyBackupPath)).industries;
    }

    if (!restoredIndustries) {
      restoredIndustries = ensureMarketSectorInIndustries(readJson(BACKUP_DATA_PATH, null));
    }

    if (!restoredIndustries) {
      return res.status(404).json({ success: false, message: 'Backup file missing from disk.' });
    }

    const nowIso = new Date().toISOString();
    const backupCreated = createBackupFromDefault(status, nowIso);
    const remainingBackups = backups.filter((backup) => backup.id !== targetBackup.id && backup.filename !== targetBackup.filename);
    const nextBackups = backupCreated ? [backupCreated, ...remainingBackups] : remainingBackups;

    writeJson(DEFAULT_DATA_PATH, restoredIndustries);

    const nextStatus = clearTempDataset({
      ...status,
      defaultDataset: createDatasetMeta({
        filename: 'default_data.json',
        originalName: targetBackup.originalName,
        uploadedAt: targetBackup.uploadedAt || nowIso,
        persistedAt: nowIso,
        restoredAt: nowIso,
        size: Buffer.byteLength(JSON.stringify(restoredIndustries)),
      }),
      backups: nextBackups,
    });
    ...
    return res.json({
      success: true,
      message: `Restored backup "${targetBackup.originalName}" as the default dataset.`,
      industries: restoredIndustries,
      defaultDataset: nextStatus.defaultDataset,
      restoredFrom: targetBackup,
      backupCreated,
      backups: nextStatus.backups,
    });
  } catch (err) {
...
router.post('/delete-backup', protect, (req, res) => {
  const backupId = req.body.backupId || req.body.filename;
  ...
  try {
    const status = readStatus();
    const backups = status.backups || [];
    const targetBackup = backups.find((backup) => backup.id === backupId || backup.filename === backupId);
    ...
    removeFileIfExists(backupJsonPath);
    removeFileIfExists(legacyBackupPath);

    // Update status registry
    const nextBackups = backups.filter((backup) => backup.id !== targetBackup.id && backup.filename !== targetBackup.filename);
    const nextStatus = {
      ...status,
      backups: nextBackups,
    };
    ...
    writeStatus(nextStatus);
    appendOperationLog('backup_deleted', {
      id: targetBackup.id,
    ...
    return res.json({
      success: true,
      message: `Backup version "${targetBackup.originalName}" has been deleted.`,
      backups: nextStatus.backups,
    });
  } catch (err) {
    console.error('[DeleteBackup] Error:', err.message);
    return res.status(500).json({ success: false, message: `Delete failed: ${err.message}` });
  }
});

router.post('/restore-legacy', protect, (req, res) => {
  ...
  try {
    ...
    const backupIdx = registry.backups.findIndex(b => b.filename === filename);
    if (backupIdx === -1) {
      return res.status(404).json({ success: false, message: 'Backup not found in registry' });
    }

    const backup = registry.backups[backupIdx];
    ...
    // Step 1: Move current active to backup
    if (registry.active) {
      const currentActivePath = path.join(ACTIVE_DIR, registry.active.filename);
      if (fs.existsSync(currentActivePath)) {
        const newBackupFilename = `${safeTimestamp()}_${registry.active.originalName}`;
        fs.copyFileSync(currentActivePath, path.join(BACKUP_DIR, newBackupFilename));
    ...
    // Step 2: Copy selected backup to active directory
    const newActivePath = path.join(ACTIVE_DIR, backup.originalName);
    fs.copyFileSync(backupPath, newActivePath);

    // Step 3: Parse and build dashboard data from restored file
    const buffer = fs.readFileSync(newActivePath);
    const workbook = XLSX.read(buffer, { type: 'buffer', cellDates: true });
    const allData = {};
    for (const sheetName of workbook.SheetNames) {
      allData[sheetName] = extractSeries(workbook.Sheets[sheetName]);
    }
    const industries = buildDashboard(allData);

    // Step 4: Update registry — remove restored backup, set new active
    registry.active = {
    ...
    registry.backups = registry.backups.filter(b => b.filename !== filename);
    writeRegistry(registry);

    return res.json({
      success: true,
      message: `Restored "${backup.originalName}" (uploaded ${new Date(backup.uploadedAt).toLocaleString()})`,
      restoredFrom: backup,
      industries,
    });
  } catch (err) {
    console.error('[FileRestore] Error:', err.message);
    return res.status(500).json({ success: false, message: `Restore failed: ${err.message}` });
  }
});
...
module.exports = { router, parseWorkbookBuffer, ensureMarketSectorInIndustries };
